...
/**
 * Esta classe lê teclas específicas do teclado e, para cada tecla pressionada,
 * retorna uma ação correspondente do enum ActionPressed.
 * 
 * @author Pablo Sampaio
 */
class KeyboardController implements KeyEventDispatcher {
    private KeyActionInfo[] keys;
    private int nextKey; //para implementar um revezamento -- como "getAction()" retorna apenas uma aï¿½ï¿½o, esse revezamento 
                         //evita que retorne sempre a mesma, caso mais de uma tecla seja pressionada
    ...
    public synchronized ActionPressed getAction() {
        ...
        //testa todas as teclas, partindo da prï¿½xima (nextKey)
	    for (int i = 0; i < keys.length; i++) {
    ...
    class KeyActionInfo {
    	...
    	private final boolean fireOnce; //quando verdade, se pressionar por muito tempo dispara só uma vez a ação
